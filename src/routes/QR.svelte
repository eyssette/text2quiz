	...
	<div class="box block" class:quiz-success={validate && correctAnswer==answer} class:quiz-error={validate && answer>0 && correctAnswer!=answer}>
		...
		<div class="control is-size-5 is-size-6-mobile">
			{#each textAnswersArray as textAnswer, i}
				<label class="radio" class:r-success={validate && correctAnswer==i+1 && answer==i+1} class:r-error={validate && correctAnswer!=i+1 && answer==i+1} for="quiz-q{quizId}-r{i+1}"><input type="radio" name="quiz-q{quizId}" id="quiz-q{quizId}-r{i+1}" {disabled} bind:group={answer}  value={i+1}>{@html sanitizeHTML(textAnswer)} </label>
			{/each}
